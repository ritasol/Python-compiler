...

{
    ...
    function calculateSpaceNeeded(value, variable) {
        ...
        if (getVariable({value:variable}).type && getVariable({value:variable}).type.includes('variable')) {
            ...
            //its a string concatenation, find the length of the combined string
            console.log("calculateSpace>", "finding length of combined:", value.properties.addedStrings)
            let spaceNeeded = 0
            ...
            console.log("calculateSpace>", "final var", variables)
    ...
    function assignValueToVariable(variable, type, value) {
        ...
        if (getVariable({value:variable}).type === undefined) {
            ...
            if (type.includes("variable")) {
                ...
                console.log("assignValueToVariable>", "type is of string or int", value)
                if (variableIndex !== -1) {
                    newVal =
                        type === "string" ? `${variable}: \t.asciiz\t"${value}"` :
                            type === "int" ? `${variable}: \t.word\t${typeof value === "number" ? value : 0} ` :
                                "noTypeYet";
 ...
 FunctionDeclaration
  ...
  ArrayOperation
   ...
   / properties:ArrayFunction {return {token:"arrayOperation", type:"arrayLength", properties} }
  ...
  Comment
      = value:$("#" _ ((!"\n" .) _)* _) { return {token: "comment", value: value.slice(1)} }
  ...
  Variable
   = !ReservedToken name:VariableName 
  {
    return {type:name.type, value: name.value}	
  }
  ...
  ArtihmeticExpression
    = value:ArtihmeticStart {if (value.type) {
         ...
         if(value.type  === "variable") {
            ...
            if(currentType) {
                type = currentType.includes("variable-") 
                   ? currentType.slice(currentType.indexOf("variable-"))
  ...
  ArtihmeticTerm
    = head:ArtihmeticFactor tail:(_ ("*" / "//" / "/" / "%") _ ArtihmeticFactor)* {
        return buildArtihmeticExpression(head, tail)
      }
